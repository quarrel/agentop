...
Every roughly one second, continue a bounded recursive scan of the sessions directory for new rollout filenames as the recovery path. In addition, when an already-tailed rollout contains an exactly correlated `spawn_agent` result with a canonical task path, retain only that bounded path and the result timestamp as a discovery hint. Before the recursive scan's next slice, prioritise the corresponding `YYYY/MM/DD` sessions directory. The hint accelerates filename discovery but is not agent identity or lifecycle evidence: admission still requires a complete, valid `session_meta` record with the displayed session's exact `session_id`. Track discovery as `pending` until that metadata is available; discovering a filename is not enough to mark it admitted.
...
Map the small set of typed items/events to compact row/detail text. For tool calls, produce bounded deterministic action summaries from known direct arguments and from `tools.<name>(...)` occurrences inside Codex's `exec` orchestration source. Group repeated wrapped tools into call counts, show an allowlisted query, path, target, or command for a single call, and fall back to the outer tool name for unknown shapes. This is lexical summarisation only: never execute the source, and never retain raw inputs or outputs.
